...
class FatBurnChallengeGameView @JvmOverloads constructor(
    context: Context,
    ...
    fun registerPunch(
        ...
        val phase = currentPhaseWindow(elapsedMs)?.phase ?: return
        if (!phase.hittable || !isSideHittable(handSide, elapsedMs)) {
            return
        }
        totalHits += 1
        validHits += 1
        ...
        val phaseFactor = if (phase.hittable) phase.cueStrength else 0.92f
        ...
        createHitBurst(side = handSide, intensity = intensity, hit = true)
    ...
    fun previewPunch(hand: BoxingBleManager.BoxingHand? = null) {
        ...
        val handSide = hand.toPunchSide() ?: return
        val elapsedMs = SystemClock.elapsedRealtime() - sessionStartElapsedMs
        if (!isSideHittable(handSide, elapsedMs)) {
            return
        }
    ...
    private fun drawCenterBeatOrb(
        ...
        canvas.drawText(
            if (sessionActive) text("击拳", "PUNCH") else text("准备", "READY"),
            centerX,
    ...
    private fun drawCenterCopy(
        ...
        if (!sessionActive) {
            ...
            canvas.drawText(sessionConfig?.title(languageCode).orEmpty(), centerX, titleY, textPaint)

            metricPaint.color = Color.parseColor("#DBF3FF")
            metricPaint.textSize = safeWidth * 0.015f
            canvas.drawText(text("30天结构化HIT燃脂拳击锻炼计划", "30-day structured HIT fat-burning boxing program"), centerX, titleY + metricPaint.textSize * 1.9f, metricPaint)
        }

        metricPaint.color = Color.parseColor("#FBE7B0")
        metricPaint.textSize = safeWidth * 0.017f
        canvas.drawText(text("根据闪烁击拳", "Punch with the flash"), centerX, safeHeight * 0.56f, metricPaint)

        if (sessionActive && currentWindow != null) {
            metricPaint.color = Color.parseColor("#DBF3FF")
            metricPaint.textSize = safeWidth * 0.014f
            canvas.drawText("${currentWindow.phase.label(languageCode)} · ${beatFitLabel()}", centerX, safeHeight * 0.18f, metricPaint)
        }
    }

    private fun spawnCuePulse(cue: BeatCue) {
        val color = if (cue.hittable) Color.parseColor("#FFD060") else Color.parseColor("#8AD8F0")
        pulses +=
            Pulse(
                side = cue.side,
                bornAtMs = SystemClock.elapsedRealtime(),
    ...
    private fun isSideHittable(
        side: Int,
        elapsedMs: Long,
    ): Boolean =
        beatCues.any { cue ->
            if (!cue.hittable || cue.side != side) {
                false
            } else {
                val offsetMs = (elapsedMs - cue.timeMs).toFloat()
                offsetMs in -SIDE_HIT_WINDOW_EARLY_MS..SIDE_HIT_WINDOW_LATE_MS
            }
        }
    ...
    private fun beatFitLabel(): String {
        if (!sessionActive || totalHits <= 0) {
            return text("等待击拳", "Ready to punch")
        }
        val hps = currentHps()
        return when {
            hps >= 2.4f -> text("爆发输出", "Explosive pace")
            hps >= 1.6f -> text("频率稳定", "Steady pace")
            hps >= 0.9f -> text("持续推进", "Keep pushing")
            else -> text("继续提速", "Build more speed")
        }
    }
    ...
    private fun idleHint(): String = text("准备进入今日燃脂课程", "Ready for today's fat-burning class")
    private fun armingHint(): String = text("正在校准蓝牙设备与训练环境", "Preparing Bluetooth hit detection")
    private fun calibratingHint(): String = text("环境校准中", "Calibrating environment")
    private fun readyHint(): String = text("根据闪烁击拳", "Punch with the flash")
    private fun waitingHint(): String = text("根据闪烁持续击拳", "Keep punching with the flash")
    private fun successHint(): String = text("已识别有效击拳", "Valid punch detected")
    private fun missHint(): String = text("继续保持输出", "Keep the output going")
    private fun recoverHint(): String = text("恢复呼吸，准备下一轮", "Recover and prepare for the next round")
    private fun offBeatHint(): String = text("继续保持输出", "Keep the output going")
    private fun permissionHint(): String = text("需要蓝牙设备权限才能开始课程", "Bluetooth connection is required")
    private fun errorHint(): String = text("蓝牙击中识别暂时中断，正在恢复", "Bluetooth hit detection interrupted, recovering")
    private fun finishedHint(): String = text("今日课程完成", "Today's class is complete")
    ...
    private companion object {
        const val SIDE_HIT_WINDOW_EARLY_MS = 620f
        const val SIDE_HIT_WINDOW_LATE_MS = 1_450f
    }
}
