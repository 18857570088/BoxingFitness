...
class ColorGraffitiActivity : AppCompatActivity() {
    ...
    private enum class GraffitiVisualMode(
        val zhLabel: String,
        val enLabel: String,
    ) {
        DarkNight("⚫ 暗夜爆彩", "⚫ Dark Splash"),
        WhiteCanvas("⚪ 纯白画布", "⚪ White Canvas"),
        RainbowLand("🌈 彩虹乐园", "🌈 Rainbow Park"),
        ;
    ...
    private lateinit var gameView: ColorGraffitiGameView
    ...
    private lateinit var graffitiQuickOptionsView: View
    ...
    private var selectedSpeed = ColorGraffitiGameView.SpeedPreset.Medium
    private var selectedDensity = ColorGraffitiGameView.DensityPreset.Duo
    private var selectedPersonaIndex = 2
    private var selectedDuration = TrainingDurationPreset.Seconds30
    private var selectedVisualMode = GraffitiVisualMode.DarkNight
    ...
    private val speedButtons = LinkedHashMap<ColorGraffitiGameView.SpeedPreset, TextView>()
    private val densityButtons = LinkedHashMap<ColorGraffitiGameView.DensityPreset, TextView>()
    private val durationButtons = LinkedHashMap<TrainingDurationPreset, TextView>()
    private val graffitiModeButtons = LinkedHashMap<GraffitiVisualMode, TextView>()
    private val personaCards = ArrayList<LinearLayout>()
    private val personaSheet: Bitmap? by lazy(LazyThreadSafetyMode.NONE) { ColorGraffitiPersonaCatalog.loadSprite(this) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        runCatching {
            ...
            renderFatalFallback(
                freeText("\u8272\u5F69\u6D82\u9E26\u542F\u52A8\u5931\u8D25\uFF0C\u5DF2\u5207\u6362\u5230\u5B89\u5168\u6A21\u5F0F\u3002", "Color Graffiti failed to start and switched to safe mode."),
            )
        }
    }
    ...
    private fun buildContentView(): View {
        ...
        gameView =
            ColorGraffitiGameView(this).apply {
                setLanguageCode(languageCode)
                setFatalErrorReporter { throwable ->
                    runOnUiThread {
                        reportFatalError("gameView", throwable)
                        renderFatalFallback(
                            freeText("\u8272\u5F69\u6D82\u9E26\u6E32\u67D3\u5931\u8D25\uFF0C\u5DF2\u5207\u6362\u5230\u5B89\u5168\u6A21\u5F0F\u3002", "Color Graffiti rendering failed and switched to safe mode."),
                        )
                    }
                }
            }
        ...
        gameView.bind(
            object : ColorGraffitiGameView.Listener {
                override fun onOverlaySnapshot(snapshot: ColorGraffitiGameView.OverlaySnapshot) {
                    runOnUiThread {
    ...
    private fun buildTopBar(): View =
        FrameLayout(this).apply {
            ...
            addView(
                buildGraffitiQuickOptions(),
                FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, dp(56), Gravity.CENTER_HORIZONTAL or Gravity.TOP).apply {
                    topMargin = dp(14)
                },
            )
        }

    private fun buildGraffitiQuickOptions(): View =
        LinearLayout(this).apply {
            graffitiQuickOptionsView = this
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
            background = glassPanel(18)
            setPadding(dp(8), dp(6), dp(8), dp(6))
            addView(graffitiQuickButton(TrainingDurationPreset.Seconds30))
            addView(horizontalSpace(dp(6)))
            addView(graffitiQuickButton(TrainingDurationPreset.Seconds60))
            addView(horizontalSpace(dp(10)))
            addView(graffitiQuickButton(GraffitiVisualMode.DarkNight))
            addView(horizontalSpace(dp(6)))
            addView(graffitiQuickButton(GraffitiVisualMode.WhiteCanvas))
            addView(horizontalSpace(dp(6)))
            addView(graffitiQuickButton(GraffitiVisualMode.RainbowLand))
        }

    private fun graffitiQuickButton(value: Any): TextView =
        TextView(this).apply {
            ...
            when (value) {
                is TrainingDurationPreset -> {
                    durationButtons[value] = this
                    text = value.label(languageCode)
                    setOnClickListener {
                ...
                is GraffitiVisualMode -> {
                    graffitiModeButtons[value] = this
                    text = value.label(languageCode)
                    setOnClickListener {
                        if (trainingActive || launchCountdownActive) {
                            return@setOnClickListener
                        }
                        selectedVisualMode = value
                        gameView.setVisualMode(value.ordinal)
                        updateGraffitiModeButtons(value)
                    }
                }
            }
        }
    ...
    private fun meterScaleItem(
        ...
        LinearLayout(this).apply {
            ...
            val labelView =
                TextView(this@ColorGraffitiActivity).apply {
                    text = value.toString()
            ...
            val tickView =
                View(this@ColorGraffitiActivity).apply {
    ...
    private fun emotionThermometerBody(): View =
        FrameLayout(this).apply {
            ...
            addView(
                LinearLayout(this@ColorGraffitiActivity).apply {
                    orientation = LinearLayout.HORIZONTAL
                    gravity = Gravity.CENTER
    ...
    private fun combatMetricChip(
        ...
        LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            gravity = Gravity.CENTER
            background = roundedFill("#16FFFFFF", 12).apply { setStroke(dp(1), Color.parseColor("#24FFFFFF")) }
    ...
    private fun buildCoachHint(): View =
        TextView(this).apply {
            coachHintView = this
            text = freeText("\u81EA\u7531\u51FA\u62F3\u5373\u53EF\u5728\u5899\u9762\u70B8\u5F00\u7EFE\u4E3D\u9713\u8679\u6CB9\u6F06\uFF0C\u58F0\u97F3\u8D8A\u54CD\uFF0C\u6E85\u5C04\u8D8A\u5927\u3002", "Punch freely to burst neon paint onto the wall. Louder hits create wider splashes.")
            gravity = Gravity.CENTER
    ...
    private fun statusCard(): View =
        LinearLayout(this).apply {
            ...
            val track =
                FrameLayout(this@ColorGraffitiActivity).apply {
        ...
        ScrollView(this).apply {
            ...
            addView(
                LinearLayout(this@ColorGraffitiActivity).apply {
                    ...
                    addView(settingsChoiceSection(freeText("\u6C14\u6CE1\u901F\u5EA6", "Bubble Speed"), ColorGraffitiGameView.SpeedPreset.values().toList()))
                    addView(verticalSpace(dp(14)))
                    addView(settingsChoiceSection(freeText("\u6C14\u6CE1\u6570\u91CF", "Bubble Count"), ColorGraffitiGameView.DensityPreset.values().toList()))
                    addView(verticalSpace(dp(14)))
    ...
    private fun settingsHeader(): View =
        LinearLayout(this).apply {
            ...
            addView(
                TextView(this@ColorGraffitiActivity).apply {
                    text = freeText("\u81EA\u7531\u62F3\u51FB\u8BBE\u7F6E", "Color Graffiti Settings")
                    gravity = Gravity.START
                    setTextColor(Color.WHITE)
                    setTypeface(Typeface.DEFAULT_BOLD)
                    textSize = 20f
    ...
    private fun segmentedButton(value: Any): TextView {
        ...
        when (value) {
            is ColorGraffitiGameView.SpeedPreset -> {
                ...
                if (value == ColorGraffitiGameView.SpeedPreset.Medium) {
                    updateSpeedButtons(value)
                }
            }

            is ColorGraffitiGameView.DensityPreset -> {
                densityButtons[value] = textView
                textView.text =
                    when (value) {
                        ColorGraffitiGameView.DensityPreset.Solo -> freeText("\u5C11", "Low")
                        ColorGraffitiGameView.DensityPreset.Duo -> freeText("\u4E2D", "Medium")
                        ColorGraffitiGameView.DensityPreset.Storm -> freeText("\u591A", "High")
                    }
                textView.setOnClickListener {
                ...
                if (value == ColorGraffitiGameView.DensityPreset.Duo) {
                    updateDensityButtons(value)
                }
            }
    ...
    private fun personaRow(
        ...
        LinearLayout(this).apply {
            ...
            repeat(columns) { column ->
                ...
                if (index >= ColorGraffitiPersonaCatalog.options.size) {
                    return@repeat
                }
                addView(personaCard(index))
                if (column < columns - 1 && index < ColorGraffitiPersonaCatalog.options.lastIndex) {
                    addView(horizontalSpace(dp(10)))
                }
            }
        }
    ...
    private fun personaCard(index: Int): LinearLayout {
        return LinearLayout(this).apply {
            ...
            personaCards += this
            addView(
                ImageView(this@ColorGraffitiActivity).apply {
                    scaleType = ImageView.ScaleType.CENTER_CROP
                    background = roundedFill("#24FFFFFF", 14)
                    setImageBitmap(ColorGraffitiPersonaCatalog.thumbnail(personaSheet, index, dp(68)))
        ...
        LinearLayout(this).apply {
            ...
            addView(
                TextView(this@ColorGraffitiActivity).apply {
                    ...
                    setTypeface(Typeface.DEFAULT_BOLD)
                    textSize = 30f
                    text = if (label == freeText("\u901F\u5EA6", "Pace")) ColorGraffitiGameView.SpeedPreset.Medium.label(languageCode) else "0"
                    setPadding(0, dp(6), 0, 0)
                    targetRef(this)
                },
            )
    ...
    private fun speakLaunchCountdownStep(step: String) {
        ...
        if (speechReady) {
            speechEngine?.speak(speechText, TextToSpeech.QUEUE_FLUSH, null, "color_graffiti_countdown_$step")
        }
    }
    ...
    private fun speakMotivationCue(
        ...
        speechEngine?.speak(message, TextToSpeech.QUEUE_FLUSH, null, "color_graffiti_motivation_$now")
    }
    ...
    private fun showTrainingSummaryPopup() {
        ...
        val whiteCanvasMode = selectedVisualMode == GraffitiVisualMode.WhiteCanvas
        val rainbowMode = selectedVisualMode == GraffitiVisualMode.RainbowLand
        val encouragement =
            when {
                hits >= 60 && rainbowMode -> freeText("\u592A\u68D2\u4E86\uFF01\u4F60\u5DF2\u7ECF\u628A\u5F69\u8679\u4E50\u56ED\u70B8\u6210\u4E86\u4E00\u5E45\u7EDA\u70C2\u7684\u5F69\u8679\u62BD\u8C61\u753B\u4F5C\u3002", "Amazing work. You turned Rainbow Park into a brilliant abstract rainbow painting.")
                hits >= 60 && whiteCanvasMode -> freeText("\u592A\u68D2\u4E86\uFF01\u4F60\u5DF2\u7ECF\u628A\u7EAF\u767D\u753B\u5E03\u6D82\u6210\u4E86\u5145\u6EE1\u751F\u547D\u529B\u7684\u9713\u8679\u753B\u4F5C\u3002", "Amazing work. You turned the white canvas into a vivid neon painting.")
                hits >= 60 -> freeText("\u592A\u68D2\u4E86\uFF01\u4F60\u5DF2\u7ECF\u628A\u753B\u5E03\u6D82\u6210\u4E86\u5145\u6EE1\u529B\u91CF\u7684\u9713\u8679\u753B\u4F5C\u3002", "Amazing work. You turned the canvas into a powerful neon painting.")
                hits >= 30 && rainbowMode -> freeText("\u5F88\u597D\uFF01\u5F69\u8679\u5F27\u7EBF\u5DF2\u7ECF\u5F00\u59CB\u5C42\u5C42\u53E0\u52A0\uFF0C\u6574\u4E2A\u4E50\u56ED\u90FD\u88AB\u4F60\u70B9\u4EAE\u4E86\u3002", "Great round. Rainbow arcs are layering beautifully and lighting up the whole park.")
                hits >= 30 && whiteCanvasMode -> freeText("\u5F88\u597D\uFF01\u7EAF\u767D\u753B\u5E03\u4E0A\u7684\u989C\u8272\u5C42\u6B21\u8D8A\u6765\u8D8A\u4E30\u5BCC\u4E86\u3002", "Great round. The white canvas is building beautiful layers of color.")
                hits >= 30 -> freeText("\u5F88\u597D\uFF01\u989C\u8272\u5DF2\u7ECF\u5F00\u59CB\u5C42\u5C42\u53E0\u52A0\uFF0C\u518D\u6765\u4E00\u8F6E\u4F1A\u66F4\u7EDA\u70C2\u3002", "Great round. The colors are layering beautifully, and the next round will look even richer.")
                hits > 0 && rainbowMode -> freeText("\u4E0D\u9519\uFF01\u4F60\u5DF2\u7ECF\u8BA9\u7B2C\u4E00\u9053\u5F69\u8679\u5728\u4E50\u56ED\u80CC\u666F\u4E0A\u7EFD\u653E\u4E86\u3002", "Nice start. Your first rainbow arc already brought the park backdrop to life.")
                hits > 0 && whiteCanvasMode -> freeText("\u4E0D\u9519\uFF01\u4F60\u7684\u7B2C\u4E00\u5C42\u989C\u8272\u5DF2\u7ECF\u8BA9\u7EAF\u767D\u753B\u5E03\u6D3B\u4E86\u8D77\u6765\u3002", "Nice start. Your first layer of color has already brought the white canvas to life.")
                hits > 0 -> freeText("\u8FD9\u4E00\u8F6E\u5DF2\u7ECF\u8BA9\u753B\u5E03\u5F00\u59CB\u8FC7\u6E21\u51FA\u8272\u5F69\uFF0C\u7EE7\u7EED\u753B\u4E0B\u53BB\u5427\u3002", "Nice start. The canvas is already filling with color. Keep painting.")
                else -> freeText("\u8FD9\u4E00\u8F6E\u7ED3\u675F\u4E86\uFF0C\u4E0B\u6B21\u76F4\u63A5\u968F\u610F\u51FA\u62F3\uFF0C\u8BA9\u989C\u8272\u81EA\u7136\u70B8\u5F00\u3002", "This round is done. Next time, just punch freely and let the colors explode.")
            }
        evaluationTitleView.text = freeText("\u672C\u8F6E\u8272\u5F69\u56DE\u987E", "Color Round Summary")
        ...
        speakMotivationCue(
            freeText(
                if (rainbowMode) {
                    "\u672C\u8F6E\u5B8C\u6210\uFF0C\u538B\u529B\u503C\u4E0B\u964D\u5230 $stressValue\uFF0C\u5E73\u9759\u503C\u4E0A\u5347\u5230 $calmValue\u3002\u4F60\u7684\u5F69\u8679\u4E50\u56ED\u753B\u4F5C\u771F\u7EDA\u70C2\uFF01"
                } else if (whiteCanvasMode) {
                    "\u672C\u8F6E\u5B8C\u6210\uFF0C\u538B\u529B\u503C\u4E0B\u964D\u5230 $stressValue\uFF0C\u5E73\u9759\u503C\u4E0A\u5347\u5230 $calmValue\u3002\u4F60\u7684\u7EAF\u767D\u753B\u5E03\u8D8A\u6765\u8D8A\u7CBE\u5F69\u4E86\uFF01"
                } else {
                    "\u672C\u8F6E\u5B8C\u6210\uFF0C\u538B\u529B\u503C\u4E0B\u964D\u5230 $stressValue\uFF0C\u5E73\u9759\u503C\u4E0A\u5347\u5230 $calmValue\u3002\u4F60\u7684\u753B\u5E03\u8D8A\u6765\u8D8A\u7CBE\u5F69\u4E86\uFF01"
                },
                if (rainbowMode) {
                    "Round complete. Stress is down to $stressValue, calm is up to $calmValue. Your Rainbow Park artwork looks brilliant."
                } else if (whiteCanvasMode) {
                    "Round complete. Stress is down to $stressValue, calm is up to $calmValue. Your white canvas looks more alive than ever."
                } else {
                    "Round complete. Stress is down to $stressValue, calm is up to $calmValue. Your canvas looks more alive than ever."
                },
            ),
            force = true,
        )
        ...
        runOnUiThread {
            if (!isFinishing && !safeModeActive && ::gameView.isInitialized) {
                runCatching {
                    ...
                    renderFatalFallback(
                        freeText("Color Graffiti hit feedback failed and switched to safe mode.", "Color Graffiti hit feedback failed and switched to safe mode."),
                    )
                }
            }
        }
    ...
    private fun dispatchDetectorState(
        ...
        runOnUiThread {
            ...
            if (::stateTitleView.isInitialized && ::stateBodyView.isInitialized && ::progressFillView.isInitialized) {
                stateTitleView.text =
                    when (type) {
                        "idle" -> freeText("\u81EA\u7531\u62F3\u51FB", "Color Graffiti")
                        ...
                        else -> freeText("\u81EA\u7531\u62F3\u51FB", "Color Graffiti")
                    }
    ...
    private fun updateSpeedButtons(selected: ColorGraffitiGameView.SpeedPreset) {
    ...
    private fun updateDensityButtons(selected: ColorGraffitiGameView.DensityPreset) {
    ...
    private fun updateGraffitiModeButtons(selected: GraffitiVisualMode) {
        graffitiModeButtons.forEach { (mode, view) ->
            val active = mode == selected
            view.background = segmentedBackground(active)
            view.setTextColor(if (active) Color.parseColor("#071524") else Color.parseColor("#EBF7FF"))
            view.isEnabled = !trainingActive && !launchCountdownActive
        }
    }
    ...
    private fun updateTrainingUi() {
        ...
        if (::graffitiQuickOptionsView.isInitialized) {
            graffitiQuickOptionsView.visibility = if (activeRound) View.GONE else View.VISIBLE
            graffitiQuickOptionsView.isEnabled = !activeRound
        }
    ...
    private fun renderFatalFallback(message: String) {
        ...
        host.addView(
            LinearLayout(this).apply {
                ...
                addView(
                    TextView(this@ColorGraffitiActivity).apply {
                        text = freeText("色彩涂鸦安全模式", "Color Graffiti Safe Mode")
                        gravity = Gravity.CENTER
                        setTextColor(Color.WHITE)
                        setTypeface(Typeface.DEFAULT_BOLD)
                        textSize = 22f
    ...
    private fun reportFatalError(
        ...
        runCatching {
            File(filesDir, "color_graffiti_last_error.txt").writeText(errorText, Charsets.UTF_8)
        }
    }
    ...
    companion object {
        const val EXTRA_SENSITIVITY_LEVEL = "color_graffiti_sensitivity"
        const val EXTRA_LANGUAGE = "color_graffiti_language"
        private const val MAX_DETECTOR_RECOVERY_ATTEMPTS = 3
        private const val DETECTOR_RECOVERY_DELAY_MS = 320L
        private const val DAILY_EMOTION_PREFS = "color_graffiti_daily_emotion"
